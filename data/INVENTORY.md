...
| `tier1_sets.csv` | <0.01 MB | The **20 tier-1 validation locations** of a multifamily dPL run `[set_id, name, entity_id, ref_kind, system, arcs, area_mi2, volume_scored, val_start_wy, val_end_wy, note]`: the CDEC and DWR-unimpaired training watersheds of `data/multifamily/` that map onto CalSim3 rim arcs, one arc set per watershed (the USGS creeks and the four Tulare basins have none; `cdec_BND` and `cdec_CSN` share the UF6 and UF13 sets), with its reference kind (`anchor` = the rim system's `FLOW-UNIMPAIRED` series, 10 sets; `arcsum` = the sum of the member `INFLOW` arcs, 10 sets) and `area_mi2` = the sum of the members' `CalSim3_Merged` `SQ_MI`. Hand-curated from the registry arc lists and the crosswalk, the two `val_*` columns apart: `val_start_wy`, `val_end_wy` = the first and last water year of the location's trimmed validation window, the run of at least 20 water years inside WY1950–84 in which the registry's USGS creek gauges covered the least of it (1950 and 1984 = the full window, kept where no creek reaches the location), written and checked by `python -m sacsma.dpl.calsim_windows` | `dpl.calsim_tier1` / `calsim_tier2` / `calsim_atlas` / `calsim_windows` / `calsim_compare` |
| `calsim3_arc_derivation.csv` | 0.04 MB | **How CalSim3 built the rim `INFLOW` series listed in the report's tables** (201 arcs: 121 Sacramento, 80 San Joaquin) `[arc, region, name, flow_type, method_class, method, gauge, period, record_frac, record_owner, tier1_set, notes]`, transcribed by hand from DWR's *CalSim3 Hydrology Report* (2023), Tables 5-1/5-2 (Sacramento) and 5-5/5-6 (San Joaquin): flow type, method flags, gauge and period as printed. `method_class` groups the flags into eight classes; `record_frac` = the share of WY1950–84 inside the listed gauge period; `record_owner` = `arc` (the arc's own gauge) or `donor` (the neighbour or downstream record it was split or proportioned from), blank where no gauge period falls in the window. Arc ids follow `calsim3_inflow_monthly.csv`. Hand-maintained, never auto-generated | `dpl.calsim_atlas` (tier-2 tables) |
...
| `et_obs/{terraclimate,fldas,era5land}_gee_cell_monthly.npz`, `swe_obs/{daymet,terraclimate,fldas,era5land}_swe_gee_cell_monthly.npz` | 25 MB (LFS) | The 7 GEE obs products, exported 2026-07-16 (spec v2: cell-rectangle mean @ native asset scale — terraclimate 4638 m, fldas/era5land 11132 m, daymet 1000 m; asset versions as of export, recorded in each npz `meta`). All 4410 cells × 1988-01..2018-12; NaN ≤0.05% (FLDAS land mask). `dataprep/gee_obs_region.py --products all --project ee-warnold` | dPL ET/SWE obs losses (`dpl/data.py` ET_DIR/SWE_DIR read these) |
| `et_obs/{openet,modis}_gee_cell_monthly.npz` | 12 MB (LFS) | Benchmark-only ET referees at full available span, exported 2026-07-17: OpenET ensemble (`et_ensemble_mad`, 30 m mosaic, 1999-10→2024-12, NaN 0) + MOD16A2GF (8-day ET summed monthly ×0.1, 500 m, 2000-01→2025-12, NaN 0.14% unvegetated). All 4410 cells. NOT in the training `ET_FILES` (no calibration-window coverage) | future benchmarking only |
...
| `uf_gauges.csv` | 0.002 MB | `[uf, lat, lon, gauge_source, area_mi2_swat, area_source]`, hand-maintained, the 18 arc-mapped subbasins: one pour point each (UF 7 has none by construction) and, for 16 of them, the area the report gives (14 are the Appendix A SWAT model areas of Tables A-1/A-2; UF 8 and UF 15 draw on the Chapter 3 text, as their `area_source` cells state; UF 6 and UF 7 have no usable single model). `gauge_source` names where the coordinates come from: `cdec_stations:<id>` rows copy `cdec_fnf/stations.csv` and are checked against it at build time, `USGS NWIS <gid>` rows are the NWIS site coordinates. `area_source` cites table and page of "DWR (2016) unimpaired-flow report", the report named at the top of this section. The pour point is where the subbasin's arc set drains (the dam or gauge that `dataprep/build_entities.py` takes as the training-entity outlet), so it can sit several km from the station the subbasin is named after (up to ~14 km, UF 16) |
