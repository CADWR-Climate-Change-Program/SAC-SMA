...
| `tier1_sets.csv` | <0.01 MB | The **20 tier-1 validation locations** of a multifamily dPL run `[set_id, name, entity_id, ref_kind, system, arcs, area_mi2, volume_scored, note]`: the CDEC and DWR-unimpaired training watersheds of `data/multifamily/` that map onto CalSim3 rim arcs, one arc set per watershed (the USGS creeks and the four Tulare basins have none; `cdec_BND` and `cdec_CSN` share the UF6 and UF13 sets), with its reference kind (`anchor` = the rim system's `FLOW-UNIMPAIRED` series, 10 sets; `arcsum` = the sum of the member `INFLOW` arcs, 10 sets) and `area_mi2` = the sum of the members' `CalSim3_Merged` `SQ_MI`. Hand-curated from the registry arc lists and the crosswalk | `dpl.calsim_tier1` / `calsim_tier2` / `calsim_atlas` |
