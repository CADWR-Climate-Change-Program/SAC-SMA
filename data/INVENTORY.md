...
| `tier1_sets.csv` | <0.01 MB | The **20 tier-1 validation locations** of a multifamily dPL run `[set_id, name, entity_id, ref_kind, system, arcs, area_mi2, volume_scored, note]`: each training target of `data/multifamily/` expressed as a CalSim3 arc set, with its reference kind (`anchor` = the rim system's `FLOW-UNIMPAIRED` series, 10 sets; `arcsum` = the sum of the member `INFLOW` arcs, 10 sets) and `area_mi2` = the sum of the members' `CalSim3_Merged` `SQ_MI`. Hand-curated from the registry arc lists and the crosswalk | `calsim.tier1` / `tier2` / `atlas` |
| `calsim3_arc_derivation.csv` | 0.04 MB | **How CalSim3 built each rim `INFLOW` series** (201 arcs: 121 Sacramento, 80 San Joaquin) `[arc, region, name, flow_type, method_class, method, gauge, period, record_frac, record_owner, tier1_set, notes]`, transcribed by hand from DWR's *CalSim3 Hydrology Report* (2023), Tables 5-1/5-2 (Sacramento) and 5-5/5-6 (San Joaquin): flow type, method flags, gauge and period as printed. `method_class` groups the flags into eight classes; `record_frac` = the share of WY1950–84 inside the listed gauge period; `record_owner` = `arc` (the arc's own gauge) or `donor` (the neighbour or downstream record it was split or proportioned from). Arc ids follow `calsim3_inflow_monthly.csv`. Hand-maintained, never auto-generated | `calsim.atlas` (tier-2 tables) |
