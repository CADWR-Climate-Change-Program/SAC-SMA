...
is what makes the variant chain below practical, since each physics upgrade only
...
- **Frozen scoring.** Reported skill comes from exporting the trained parameter
  ...
  counterpart, is the one exception noted in Implementation: training and scoring,
  below). The dPL system is a calibration engine; the production model remains the
  parity-verified code of Part I.
...
the reference numerics (Implementation: training and scoring, below). No smoothing of
the physics is used in any canonical run.
...
**`noah` is climate-adaptive.** The parameter network's input features are
augmented with four climate indices (mean precipitation, an aridity index, a
snow fraction, and a seasonality index), computed from the forcing and
recomputed under any (Δprecip, ΔT) perturbation. Because these indices flow
through the same network as the physiographic features, a perturbed climate
changes not only the forcing but the exported SAC-SMA parameters themselves,
a space-for-time response rather than a fixed regionalization. This costs
essentially no present-day skill (frozen cal/val 0.779/0.804 against
0.767/0.799 for the climate-frozen version) and is the physics basis every
current hybrid below builds on. The climate-frozen predecessor is kept for
lineage as `superseded/noah_noca`; it remains the physics reference for the
plain physics-ladder comparisons above and the teacher for the earlier hybrid
generation described below.

**A seasonal fine-tune, evaluated and set aside.** An intermediate rung (`noah_ft`)
warm-started from the (climate-frozen) `noah` optimum and added a bounded annual harmonic to $K_{pet}$
...
the frozen (climate-frozen) `noah` baseline, the CalSim3 comparison was a wash overall,
snow-free NHG regressed because its melt parameters are invisible to the SWE signal,
and the northern basins drifted low in volume through the water-balance band. The
seasonal parameters also have no frozen-model counterpart, which made every downstream
consumer depend on exported daily simulations instead of parameter tables. The
fine-tune was set aside; the identification finding itself carries forward to the
options in Next steps, below.

**`hybrid`, `hybrid_dt`, and `lstm` (the SAC×LSTM ensembles).** An LSTM
...
calendar-keyed mean corrections that carry unchecked into validation. Every canonical
run also carries the raw Priestley–Taylor potential (the physics' energy-demand
signal, a deterministic function of the forcing, so it recomputes exactly under any
temperature counterfactual) as a sixth dynamic channel. Each configuration is a
three-seed ensemble scored on the ensemble-mean flow.

The three configurations share that architecture and differ only in the physics
channel and the loss. `hybrid` keeps the physics-simulation channel, now `noah`'s
climate-adaptive export, with no response loss: the best-skill configuration.
`hybrid_dt` adds a **multi-anchor (Δp, ΔT) response-consistency loss** on top of the
same architecture. `lstm` drops the physics-simulation channel entirely
(`use_sim=False`), a no-physics control that keeps the PET channel and static
attributes but forwards on meteorology alone.

For `hybrid_dt`'s loss, the `noah` physics are re-run under each of 14 (Δprecip, ΔT)
anchors, $\{-20\%,-10\%,0,+10\%,+20\%\}\times\{0,+2,+4\ ^\circ\mathrm{C}\}$ (excluding
the origin), each producing its own teacher simulation through the same differentiable
pipeline that exports the input channel. During training, each batch is additionally
forwarded once per anchor with the precipitation, temperature, and PET channels
shifted and the physics channel replaced by that anchor's teacher, and the network's
response at every anchor is pulled toward the physics response by
...
the direct multi-anchor generalization of the single +2 °C teacher the predecessor
generation below used ($\mathcal{A}$ a single point, $\lambda=0.3$).

**The frozen-noah predecessor.** `hybrid` and `hybrid_dt` generalize an earlier pair
built on the climate-frozen `noah_noca` physics, retained for lineage as
`superseded/hybrid_noca` and `superseded/hybrid_dt_noca`. That pair established the
architecture above and the single-anchor version of the response loss:
`hybrid_noca` (eight-seed, no PET channel, no response loss) reached the same skill
class as the current `hybrid` (0.917/0.869, Table 5), and `hybrid_dt_noca` added the
PET channel plus a single +2 °C teacher ($\lambda_T=0.3$) at unchanged skill
(0.916/0.864) and a physics-consistent warming response (response ratio 1.04,
monthly-regime correlation 0.97). A screening across loss weights on that generation
showed the response scaling smoothly with the weight (λ of 0.1/0.3/1.0 recovering
36%/78%/92% of the physics response), which is what motivated widening the single
+2 °C anchor into the full (Δp, ΔT) surface described above.

The network keeps its within-climate skill but inherits the physics' climate
sensitivity (Climate response of the dPL chain, below). A residual coupling, in which the LSTM predicts the signed
physics error and $\hat{q} = \hat{q}^{phys} + \delta$, was benchmarked on the
frozen-noah predecessor to equivalent pooled skill but retired: under the validation
period's climate shift it re-injects a regime-conditional volume bias at the basins
where the physics were already right, and that behavior persisted under every
attempted repair (removing calendar inputs, penalizing the residual mean, and
improving the underlying physics baseline).
...
`hybrid_dt`) the response-consistency term of the Architecture section, above.
Selection is again pooled calibration KGE, and the validation years are never read.
...
2×10⁻¹³ mm/day). The physics numbers in Results, below, are frozen-model scores. The
exception is the LSTM stage of the hybrids, which has no frozen counterpart and is scored from its
...
| `pt` | grid | 0.799 | 0.826 |
| `noah` (climate-adaptive) | grid | 0.779 | 0.804 |
| `hybrid` (3-seed mean) | grid | 0.922 | 0.877 |
| `hybrid_dt` (3-seed mean) | grid | 0.873 | 0.849 |
| `lstm` (3-seed mean, no-physics control) | grid | 0.909 | 0.835 |
...
described in Implementation: training and scoring, above. The program's run log
records 0.807/0.829 for `hamon` from selection-time scoring; the frozen re-score shown
here is the apples-to-apples basis. The frozen-noah predecessor generation
(Architecture, above) is retained for lineage: `superseded/noah_noca`
0.767/0.799, `superseded/hybrid_noca` (8-seed) 0.917/0.869, `superseded/hybrid_dt_noca`
(8-seed) 0.916/0.864.
...
- **The physics upgrades trade a little skill for better structure.** PT (−0.010 val)
  and Noah-lite ET (−0.022 from PT) each give up some calibration-period skill in
  ...
  Out-of-calibration evaluation against CalSim3 and Climate response of the dPL
  chain, below, show the results of that trade.
- **Observation data can steer seasonal timing, but the package was not kept.**
  The seasonal fine-tune described in Architecture, above, demonstrated that satellite ET/SWE seasonality
  ...
  multi-timescale program in Next steps, below.
- **The hybrid closes the skill gap regardless of the physics underneath.** The
  current ensembles lift pooled validation KGE to 0.85–0.88 (Table 5), with gains
  concentrated in calibration (0.922), matching the pattern the frozen-noah
  predecessor established on its own physics basis (0.86–0.87). Two persistent
  weaknesses temper this. SCC remains a validation weak point that the network
  cannot fix from calibration-period data alone. And at basins where the physics
  already match CalSim3 (NML, MRC, ORO), the hybrid's extra skill comes with
  regime-conditional volume drift, detailed below. That drift proved intrinsic to
  calibration-only training under a shifted validation climate: it survived removal
  of the calendar inputs, penalties on the correction's mean, and an improved physics
  baseline. At those basins the physics remain the more trustworthy out-of-sample
  answer.
- **A no-physics control makes the case for keeping the physics channel.** `lstm`
  drops the physics-simulation input entirely and still reaches respectable skill
  (0.835 val) on meteorology, PET, and static attributes alone, but its +2 °C
  response is wrong-signed (Climate response of the dPL chain, below; response ratio
  −0.94). A network trained only on
  historical variability has no reason to extrapolate a temperature sensitivity, and
  without a physics channel it has nothing else to anchor one. This is the direct
  evidence for keeping the physics simulation as an input rather than trying to
  reproduce its skill from meteorology alone.
...
pooled GA's worst CalSim3-basis failures (the SHA/BND rim under-run of Part I's
Performance review largely closes on the re-footed domain), and the physics rungs hold that ground with
progressively more defensible ET. The Noah rung reaches NML and MRC alongside the PT
cascade (monthly KGE 0.94 and 0.93), which suggests the soil-moisture-limited ET
structure itself is doing the work.
...
(`noah`), 0.87 (`hybrid`), and 0.88 (`hybrid_dt`, the best of the set). The basic
hybrid's in-sample skill does not carry fully to this test at the good-physics basins
(NML 0.72, MRC 0.83 against the physics' 0.93–0.94), which is the regime-conditional
drift noted above; the response-constrained variant recovers most of that ground (NML
0.88, MRC 0.90) while improving nearly everywhere else, including a substantial repair
of NHG's seasonal shape (mismatch 13% for the physics, 3–4% for both hybrids).
...
Repeating Part I's temperature-detrending experiment (Warming sensitivity) on the dPL
variants shows the intended progression through the physics rungs. The Hamon rung inherits the Part I
...
in, and the diagnostic that shows this is a per-watershed (Δprecip, ΔT) response
surface (`sacsma.dpl.dtdp_response`), scored on the same out-of-calibration record as
above so most of the surface is genuinely held out. Four metrics are tracked per
watershed as percent change from the (0, 0) present-climate baseline: total annual
runoff, the Apr–Jul snowmelt freshet, and the daily 99.9th and 30th flow percentiles
(the flood peak and the low flow). Warming shrinks the freshet but intensifies the
flood peak, a crossover in which the same warming that reduces the seasonal snowmelt
pulse increases the extreme daily tail (snow turning to rain, and rain falling on an
existing pack).

Under an unconstrained +2 °C perturbation `hybrid` (no response loss) over-responds,
with an aggregate warming ratio of 1.50 against the physics. The no-physics `lstm`
control is wrong-signed (ratio −0.94): trained only on historical variability, it
has no reason to extrapolate a temperature sensitivity, and without a physics channel
nothing anchors one. `hybrid_dt`'s multi-anchor response loss repairs this: its
aggregate warming ratio is 1.14, right-signed at all 15 basins, at a real but modest
skill cost (val 0.849 against `hybrid`'s 0.877). Scored on the full response surface,
`hybrid_dt` reproduces the freshet-shrinks/flood-peak-intensifies crossover on tails it
was never directly trained on, while the unconstrained `lstm` control roughly doubles
the flood-peak response.

```{figure} ../artifacts/dpl/figures/hybrid_summary.png
:width: 6.2in

Figure 6. Skill and climate-response fidelity across the current hybrid family:
ensemble-mean cal/val KGE, the pooled warming-response curve, and the pooled
precip-response curve, one line per model with the `noah` physics as the black
reference.
```

The frozen-noah predecessor generation established the precedent this generalizes
from. There, a three-arm progression (`hybrid_noca`, an intermediate PET-input-only
ensemble, and `hybrid_dt_noca`) isolated a skill lever from a response lever: adding
the Priestley–Taylor potential as an input channel raised validation KGE to 0.872 (the
best of that progression) while leaving the +2 °C response nearly flat (ratio 0.24);
only the temperature-consistency loss moved the response, at a cost of about 0.008 KGE.
A screening across loss weights on that generation showed the response scaling
smoothly with the weight, which is what motivated widening a single +2 °C teacher into
the full (Δp, ΔT) surface for the current family. An updated progression figure for
the current chain (`noah` → `hybrid` → `hybrid_dt`) is in Appendix B.6.

The method has one boundary. The response loss buys a physics-consistent climate
response; it does not repair the validation-period volume drift at NML/MRC/ORO
discussed above. The two are distinct failure modes with distinct causes.
